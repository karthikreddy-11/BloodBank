<html>
<head>
<style>
/* Header style with dark blue */
.header {
  overflow: hidden;
  background-color: #0071a6; /* Dark Blue */
  top: 0;
  width: 100%;
  padding: 10px 5px;
  color: #ffffff; /* White text */
}

/* Style the header links */
.header a {
  float: left;
  color: #EDE8F5; /* Very light blue */
  text-align: center;
  padding: 12px;
  text-decoration: none;
  font-size: 18px;
  line-height: 25px;
  border-radius: 4px;
  font-weight: bold;
  transition: background-color 0.3s ease;
}

/* Style the logo link */
.header a.logo {
  font-size: 28px;
  font-weight: bold;
  color: #EDE8F5; /* Very light blue for emphasis */
}

/* Change the background color on hover */
.header a:hover {
  background-color: #7091E6; /* Medium Blue for hover */
  color: #ffffff; /* White text on hover */
}

/* Style for the active/current link */
a.act {
  background: linear-gradient(to right, #ADBBDA 0%, #8697C4 100%); /* Light Grayish to Muted Blue gradient */
  color: white;
  border-radius: 30px;
}

/* Alternate logo style when active */
a.logo2 {
  background-color: #8697C4; /* Muted Blue */
  color: #ffffff;
}

/* Float the link section to the right */
.header-right {
  float: right;
}

/* Responsive design for small screens */
@media screen and (max-width: 500px) {
  .header a {
    float: none;
    display: block;
    text-align: left;
  }
  .header-right {
    float: none;
  }
}
</style>
</head>

<body style="background-color: #EDE8F5;"> <!-- Very light blue background -->
  <div class="header">
    <a href="home.php" class="logo" <?php if($active=='home') echo "class='logo2'"; ?>>Blood Bank</a>
    <div class="header-right">
      <a href="about_us.php" <?php if($active=='about') echo "class='act'"; ?>>About Us</a>
      <a href="why_donate_blood.php" <?php if($active=='why') echo "class='act'"; ?>>Why Donate Blood</a>
      <a href="donate_blood.php" <?php if($active=='donate') echo "class='act'"; ?>>Become A Donor</a>
      <a href="need_blood.php" <?php if($active=='need') echo "class='act'"; ?>>Need Blood</a>
      <a href="contact_us.php" <?php if($active=='contact') echo "class='act'"; ?>>Contact Us</a>
    </div>
  </div>
</body>
</html>
